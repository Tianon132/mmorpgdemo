...
namespace GameServer.Services
{
    class ItemService : Singleton<ItemService>
    {
        public ItemService()
        {
            ...
            MessageDistributer<NetConnection<NetSession>>.Instance.Subscribe<ItemEquipRequest>(this.OnItemEquip);//注册装备
        }
        ...
        void OnItemBuy(NetConnection<NetSession> sender, ItemBuyRequest request)//购买的request
        {
            ...
            sender.SendResponse();//发送消息，封装好的发送
        }

        private void OnItemEquip(NetConnection<NetSession> sender, ItemEquipRequest request)//装备道具的request
        {
            Character character = sender.Session.Character;
